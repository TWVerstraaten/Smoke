...
#include "../app/thr/ThreadPool.h"
#include "../app/thr/ThreadSettings.h"
...
namespace app::fluid {
    ...
    void Fluid::set_circle() {
        for (size_t i = 0; i != g_point_count - 1; ++i) {
            for (size_t j = 0; j != g_point_count - 1; ++j) {
                if (std::sqrt(app::math::square(i - g_cell_count / 2) + app::math::square(j - g_cell_count / 2)) < g_cell_count / 4.0f &&
                    std::sqrt(app::math::square(i - g_cell_count / 2) + app::math::square(j - g_cell_count / 2)) > g_cell_count / 4.0f - 4) {
                    m_density[i][j] = 120;
                    m_u[i][j] *= 3;
                    m_v[i][j] *= 3;
                }
    ...
    static float clamp_to_grid(float value) {
        return app::math::clamp(value, 0.5f, static_cast<float>(g_cell_count) - 0.5f);
    }
    ...
    static std::pair<size_t, size_t> screen_to_array_indices(float x, float y) {
        const auto i = static_cast<size_t>(app::math::clamp(x, 0.0f, 1.0f) * (g_point_count - 1));
        const auto j = static_cast<size_t>(app::math::clamp(y, 0.0f, 1.0f) * (g_point_count - 1));
        return {i, j};
    }
