...
namespace app::disp {
    ...
    class SettingsWidget : public QWidget {
      ...
      public:
        ...
        void add(
            const QString& name, size_t& value, size_t min, size_t max = std::numeric_limits<int>::max(), const std::function<void(size_t)>& call_back = [](size_t) {});
    ...
    void SettingsWidget::add(const QString& name, Enum& the_enum, const QMap<QString, Enum>& names) {
        ...
        connect(combo_box, QOverload<const QString&>::of(&QComboBox::textActivated), [names, &the_enum](const QString& selected) { the_enum = names[selected]; });
